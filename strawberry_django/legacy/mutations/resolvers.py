...
def create(*args, types=None, pre_save=None, post_save=None):
    model, output_type, input_type = resolve_type_args(
        args, types=types, is_input=True, single=True
    )
...
def create_batch(*args, types=None, pre_save=None, post_save=None):
    ...
    @strawberry.mutation
    @django_resolver
    def mutation(info, data: List[input_type]) -> List[output_type]:
        instances = []
        for d in data:
...
def update(*args, types=None):
    ...
    @django_resolver
    def mutation(
        data: update_type, filters: Optional[List[str]] = []
    ) -> List[output_type]:
