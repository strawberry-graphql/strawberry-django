...
def mutations(*args, types=None):
    ...
    for model, output_type, input_type in type_args:
        ...
        mutation_fields[f"create_{object_name}"] = resolvers.create(
            model, output_type, input_type
        )
        mutation_fields[f"create_{object_name}s"] = resolvers.create_batch(
            model, output_type, input_type
        )
        mutation_fields[f"update_{object_name}s"] = resolvers.update(
            model, output_type, input_type
        )
        mutation_fields[f"delete_{object_name}s"] = resolvers.delete(
            model, output_type, input_type
        )
