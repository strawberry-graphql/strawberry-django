...
from strawberry.relay import Node, ListConnection
...
def test_window_reordering():
    ...
    qs = Project.objects.all().order_by('name', 'pk').annotate(
        _row_num=Window(
...
def test_cursor_pagination():
    ...
    class Query:
        ...
        def projects2(self) -> Iterable[ProjectType]:
            return Project.objects.all().annotate(foo=Upper(F('name'))).order_by(F('foo').desc())
