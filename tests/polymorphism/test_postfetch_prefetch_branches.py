...
def test_polymorphic_postfetch_prefetch_roots_from_strings():
    ...
    assert any(p["__typename"] == "ArtProjectType" for p in result.data["projects"])  # type: ignore[index]
...
def test_postfetch_unknown_relation_name_is_skipped():
    ...
    # Materialize and fetch the single result
    ...
    # Note: resolvers.py line 80 (continue when ids list is empty) is practically
    # unreachable with real QuerySet instances because saved Django model
    # instances always have a primary key; if there are no instances, we return at
    # the earlier subclass-level check. Therefore we do not try to simulate it here.
