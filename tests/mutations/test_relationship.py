...
def test_create_one_to_many(mutation, color):
    result = mutation('{ fruit: createFruit(data: { name: "strawberry", color: { set: 1 } }) { color { name } } }')
...
def test_update_many_to_one(mutation, fruit, color):
    result = mutation('{ colors: updateColors(data: { fruits: { add: [1] } }) { fruits { name } } }')
    assert not result.errors
    assert result.data['colors'] == [
        { 'fruits': [{ 'name': fruit.name }] }
    ]

    result = mutation('{ colors: updateColors(data: { fruits: { remove: [1] } }) { fruits { name } } }')
    assert not result.errors
    assert result.data['colors'] == [
        { 'fruits': [] }
    ]

    result = mutation('{ colors: updateColors(data: { fruits: { set: [1] } }) { fruits { name } } }')
    assert not result.errors
    assert result.data['colors'] == [
        { 'fruits': [{ 'name': fruit.name }] }
    ]
...
def test_create_many_to_many(mutation, fruit):
    result = mutation('{ types: createFruitType(data: { name: "Berries", fruits: { set: [1] } }) { fruits { name } } }')
    assert not result.errors
    assert result.data['types'] == { 'fruits': [{ 'name': fruit.name }] }


def test_update_many_to_many(mutation, fruit, fruit_type):
    result = mutation('{ types: updateFruitTypes(data: { fruits: { add: [1] } }) { fruits { name } } }')
    assert not result.errors
    assert result.data['types'] == [
        { 'fruits': [{ 'name': fruit.name }] }
    ]

    result = mutation('{ types: updateFruitTypes(data: { fruits: { remove: [1] } }) { fruits { name } } }')
    assert not result.errors
    assert result.data['types'] == [
        { 'fruits': [] }
    ]

    result = mutation('{ types: updateFruitTypes(data: { fruits: { set: [1] } }) { fruits { name } } }')
    assert not result.errors
    assert result.data['types'] == [
        { 'fruits': [{ 'name': fruit.name }] }
    ]

    result = mutation('{ types: updateFruitTypes(data: { fruits: { set: [] } }) { fruits { name } } }')
    assert not result.errors
    assert result.data['types'] == [
        { 'fruits': [] }
    ]
