...
def test_excessive_materialization_before_pagination_on_connection():
    ...
    Context:
      - When querying a Connection field (Relay) and selecting nested reverse
        relations on a subclass (e.g., Company -> projects -> ArtProject ->
        artNotes -> details), the optimizer lifts child postfetch hints to the
        parent accessor ("projects").
      - The queryset hook for Connection fields currently evaluates the parent
        queryset early (via default_qs_hook -> apply_postfetch), which causes
        the entire parent queryset (all Companies) to be materialized BEFORE
        the pagination (e.g., first: 1) is applied. That leads to queries that
        batch across all parents instead of only the page.

    Evidence gathered by this test:
      - We create multiple Companies, each with one ArtProject and note+detail.
      - We query `companies(first: 1)` with nested selection under projects
        sufficient to trigger lifted parent postfetch branches.
      - By capturing SQL, we assert that the projects SELECT uses
