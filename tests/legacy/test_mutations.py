...
def test_create_batch(mutation):
    result = mutation(
        '{ users: createUsers(data: [{ name: "user1" }, { name: "user2" }]) { id name } }'
    )
    assert not result.errors
...
def test_update(mutation, users):
    result = mutation(
        '{ users: updateUsers(data: { name: "me" }, filters: ["id__lte=2"]) { id name } }'
    )
    assert not result.errors
    assert result.data["users"] == [
        {"id": "1", "name": "me"},
...
def test_save_hooks(db):
    ...
    @strawberry.type
    class Mutation:
        create_user = strawberry_django.mutations.create(
            models.User, types.User, types.UserInput, pre_save=hook
        )
